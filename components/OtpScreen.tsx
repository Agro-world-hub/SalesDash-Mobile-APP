...
return (
  ...
  >
    <ScrollView
      ...
      keyboardShouldPersistTaps="handled"
    >
      <View className={`flex-1 w-full px-3 ${isDarkMode ? 'bg-gray-900' : 'bg-white'}`}>
        {/* Header */}
        <View className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} flex-row items-center h-17 shadow-lg px-1 mb-8`}>
          <BackButton navigation={navigation} />
          <Text
            style={{ fontSize: 18 }}
            className={`font-bold text-center flex-grow mr-9 text-xl ${isDarkMode ? 'text-white' : 'text-[#6C3CD1]'}`}
          >
            OTP Verification
          </Text>
        </View>
        ...
        >
          ...
          {/* Instruction Text */}
          <Text className={`text-center mt-9 font-bold text-xl ${isDarkMode ? 'text-white' : 'text-black'}`}>
            Enter Verification Code.
          </Text>
          <Text className={`text-center mt-4 ${isDarkMode ? 'text-gray-400' : 'text-[#808080]'}`}>
            We have sent a Verification Code to your Customer’s mobile number
          </Text>
          ...
          <View className="flex-row justify-center gap-3 mb-4 mt-1 px-4">
            {otp.map((digit, index) => (
              <TextInput
                key={index}
                ref={(el) => (inputRefs.current[index] = el as TextInput)}
                className={`w-12 h-12 text-lg text-center rounded-lg ${digit
                  ? "bg-[#874DDB] text-white"
                  : isDarkMode
          ...
          <View className={`justify-center items-center ${isDarkMode ? 'bg-gray-900' : 'bg-white'}`}>
            ...
            <View className="flex-row items-center justify-center mt-3">
              ...
              <TouchableOpacity disabled={resendDisabled} onPress={handleResendOtp}>
                <Text
                  className={`ml-2 font-semibold ${resendDisabled
                    ? "text-gray-500"
                    : isDarkMode
                      ? "text-purple-300"
                      : "text-[#874DDB]"
