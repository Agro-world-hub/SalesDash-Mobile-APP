    ...
    <View className={`flex-1 w-full ${isDarkMode ? 'bg-gray-900' : 'bg-white'}`}>
      {isLoading ? (
        ...
        <>
              ...
              <View className="flex-1 justify-center items-center px-4">
                <Image
                  ...
                  style={{
                    ...
                    height: hp("20%"),
                    resizeMode: "contain",
                    //tintColor: isDarkMode ? '#ccc' : undefined,
                  }}
                />
                <Text className={`text-center mt-4 font-bold text-1xl ${isDarkMode ? 'text-white' : 'text-black'}`}>
                  No Notification Yet
                </Text>
              </View>
            ) : (
              <FlatList
                ...
                renderItem={({ item }) => {
                  const itemStyle = item.readStatus
                    ...
                    : isDarkMode ? "bg-[#2a213d]" : "bg-[#F4EDFF]";
                  return (
                    <TouchableOpacity
                      onPress={() => markAsRead(item.id)}
                      activeOpacity={0.8}
                    >
                      <View className={`shadow-md p-4 mb-3 mx-3 flex-row justify-between items-center rounded-lg ${itemStyle}`}>
                        <Image
                          source={getNotificationIcon(item.title)}
                          style={{ width: 30, height: 30, tintColor: isDarkMode ? '#ccc' : undefined }}
                        />
                        <View className="flex-1 ml-5">
                          <Text className={`${isDarkMode ? 'text-white' : 'text-gray-800'} font-bold`}>
                            {item.title}
                          </Text>
          ...
          >
            <View className="flex-1 justify-end bg-[#00000033]">
              <View className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} w-full p-2 rounded-t-lg mr-4`}>
                <View className="flex-row justify-between mt-3">
                  <TouchableOpacity
                    className="flex-1 p-2 rounded-lg mr-4"
                    onPress={deleteNotification}
                  >
                    <View className="flex-row">
                      ...
                      />
                      <Text className={`ml-4 font-bold ${isDarkMode ? 'text-white' : 'text-black'}`}>
                        Remove this notification
                      </Text>
                    </View>
                  </TouchableOpacity>
